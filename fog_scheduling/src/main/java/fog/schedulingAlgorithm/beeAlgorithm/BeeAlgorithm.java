package fog.schedulingAlgorithm.beeAlgorithm;
...
public class BeeAlgorithm {
    ...
    private double calcCost(Cloudlet cloudlet, FogDevice fogDevice) {
        ...
        cost += fogDevice.getCostPerSec() * cloudlet.getCloudletLength( ) / fogDevice.getMips();
        // cost includes the memory cost
        cost += fogDevice.getCostPerMem() * cloudlet.getMemRequired();
        // cost includes the bandwidth cost
        cost += fogDevice.getCostPerBw() * (cloudlet.getCloudletFileSize()  + cloudlet.getCloudletOutputSize());
        return cost;
    }
    ...
    public double calcFitness(Individual individual, List<FogDevice> fogDevices, List<? extends Cloudlet> cloudletList) {
        ...
        for(FogDevice fogDevice : fogDevices) {
            ...
            execTime = totalLength / fogDevice.getMips();
